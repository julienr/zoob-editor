        ...
        public void run(AccountManagerFuture<Bundle> future) {
          try {
            ...
            if (result.containsKey(AccountManager.KEY_INTENT)) {
              ...
              activity.startActivityForResult(intent, EditorConstants.GET_LOGIN);
              return;
            }

            authToken = result.getString(
                AccountManager.KEY_AUTHTOKEN);
            Log.e(EditorConstants.TAG, "Got auth token.");
            runWhenFinished();
          } catch (OperationCanceledException e) {
            Log.e(EditorConstants.TAG, "Operation Canceled", e);
          } catch (IOException e) {
            Log.e(EditorConstants.TAG, "IOException", e);
          } catch (AuthenticatorException e) {
            Log.e(EditorConstants.TAG, "Authentication Failed", e);
          }
        }
    }, null /* handler */);
  }
  ...
  public boolean authResult(int resultCode, Intent results) {
    if (results != null) {
      authToken = results.getStringExtra(
          AccountManager.KEY_AUTHTOKEN);
      Log.w(EditorConstants.TAG, "authResult: " + authToken);
    } else {
      Log.e(EditorConstants.TAG, "No auth result results!!");
    }
    runWhenFinished();
    ...
    activity.runOnUiThread(new Runnable() {
      public void run() {
        accountManager.invalidateAuthToken(EditorConstants.ACCOUNT_TYPE,
            authToken);
        UploadActivity.getInstance().getAccountChooser().chooseAccount(activity,
